...
class _searchScreenState extends State<searchScreen> {
  ...
  DetailsResult?
      posicaoEnd; // variavel global definida no globals (coisa de psicopata)
              ...
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                  return ListTile(
                    ...
                    onTap: () async {
                      final placeId =
                          recomendados[index].placeId!; // pega id do local
                      final detalhes = await googlePlace.details
                          .get(placeId); // usa a api para retornar os detalhes
                      if (detalhes != null && detalhes.result != null) {
                        if (focusNode.hasFocus) {
                          setState(
                            () {
                              posicaoEnd = detalhes.result;
                              print(posicaoEnd?.geometry?.location?.lat);

                              searchFieldController.text =
                                  posicaoEnd!.formattedAddress!;
